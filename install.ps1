...
function Find-Python {
    ...
    $pythonCmds = @('python', 'python3', 'py', 'py -3', 'py -3.11', 'py -3.10', 'py -3.9', 'py -3.8', 'py -3.7')
...
function Install-PythonDependencies {
    ...
    try {
        & $PythonCmd -c 'import aiohttp; import brotli' 2>$null
        if ($LASTEXITCODE -eq 0) {
    ...
    $pipCmds = @("$PythonCmd -m pip", 'pip', 'pip3', 'py -m pip')
    foreach ($pipCmd in $pipCmds) {
        try {
            Write-Info "  Trying: $pipCmd install aiohttp brotli"
            Invoke-Expression "$pipCmd install aiohttp brotli --quiet" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) {
...
function Main {
    ...
    Test-SourceFiles -SrcDir $srcDir
    if ($MpvPath) {
        $mpvDir = $MpvPath
    } else {
    ...
    Copy-PluginFiles -SrcDir $srcDir -MpvDir $mpvDir
